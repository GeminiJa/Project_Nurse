...
function Sidebar() {
  const router = useRouter(); // ใช้สำหรับเปลี่ยนเส้นทางการนำทาง (navigation)
  const pathname = usePathname(); // ใช้สำหรับตรวจสอบ URL ปัจจุบัน
  const [isCollapsed, setIsCollapsed] = useState(false); // State สำหรับกำหนดการยุบ-ขยาย Sidebar

  // รายการเมนูของ Sidebar พร้อมไอคอน และเส้นทาง URL ที่เกี่ยวข้อง
  const menuItems = [
    { path: "/student/student_1", label: "แสดงหลักสูตร", icon: <FaHome /> },
    { path: "/student/student_2", label: "บันทึกข้อมูล", icon: <FaBookMedical /> },
    { path: "/student/student_3", label: "ตรวจสอบรายการบันทึก", icon: <GoChecklist /> },
    { path: "/student/student_4", label: "สั่งออกข้อมูล", icon: <MdOutlineManageSearch /> },
    { path: "/logout", label: "ออกจากระบบ", icon: <MdLogout />, isLogout: true }, // เพิ่มตัวแปร `isLogout` สำหรับกำหนดสีปุ่มออกจากระบบ
  ];
    ...
    >
      ...
      <div className="space-y-4">
        {menuItems.map(({ path, label, icon, isLogout }) => (
          <button
            key={path}
            onClick={() => router.push(path)} // เมื่อกดปุ่มจะนำทางไปยัง URL ที่กำหนด
            className={`
              flex items-center gap-4 w-full p-3 rounded-lg shadow-lg transition focus:outline-none 
              ${pathname.startsWith(path) ? "bg-[#da935a] text-white" : 
                `bg-[#444444] ${isLogout ? "text-red-600" : "text-white"} 
                hover:bg-[#da935a] hover:text-black`}
              ${isCollapsed ? "justify-center" : "justify-start"}
            `}
          >
            {/* แสดงไอคอนของเมนู */}
            <div className="text-xl">{icon}</div>
            {/* แสดงข้อความของเมนู เฉพาะเมื่อ Sidebar ไม่ถูกยุบ */}
            {!isCollapsed && <span className="text-sm">{label}</span>}
          </button>
        ))}
      </div>
